...
class VwCheckListNode extends StatefulWidget {
  VwCheckListNode(
      {super.key,
...
class VwCheckListNodeState extends State<VwCheckListNode> {
  late Key stateKey;
  ...
  void implementUpdateSelectedState(bool selected, VwNode node) {
    if (selected == true) {
      ...
      if(this.widget.fieldValue.valueTypeId==VwFieldValue.vatValueLinkNodeList)
        {
          NodeUtil.removeNodeFromLinkNodeList(
              node.recordId, widget.fieldValue.valueLinkNodeList!);
        }
      else  if(this.widget.fieldValue.valueTypeId==VwFieldValue.vatNodeList){
        NodeUtil.removeNodeFromNodeList(
            node.recordId, widget.fieldValue.valueNodeList!);
  ...
  List<String> createExcludedNodeList() {
    ...
    for (int la = 0; la < widget.fieldValue.valueLinkNodeList!.length; la++) {
      try {
        final currentNodeId = widget.fieldValue.valueLinkNodeList!
            .elementAt(la)
            .nodeId;
      ...
      la++) {
        ...
        if (currentNode != null) {
          Widget currentLinkNodeWidget = VwCheckListLinkNodeRowViewer(
            ...
            selectedList: this._createSelectedList(),
  ...
  Widget implementNodeRowViewerChooseRecord({required VwNode renderedNode,
  ...
  Widget implementNodeRowViewerSelectedRecord({required VwNode renderedNode,
    required BuildContext context,
    required int index,
    Widget? topRowWidget,
    String? highlightedText,
  ...
  Widget _getFloatingActiobButtonChooseRecord({required BuildContext context,
    ...
    return FloatingActionButton.small(
        ...
        onPressed: () async{
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => createNewNode(context)),
          );
        });
  }

  Widget _getFloatingActionButtonNewRecord({required BuildContext context,
    required VwAppInstanceParam appInstanceParam,
    SyncNodeToParentFunction? syncNodeToParentFunction,
    RefreshDataOnParentFunction? refreshDataOnParentFunction}) {
/*
    return SpeedDial (
      ...
      speedDialChildren: <SpeedDialChild>[
        SpeedDialChild(
          ...
          label: 'Record Baru',
          onPressed: () {
            Widget? currentFormPage=newRowDataFormPage(syncNodeToParentFunction);
        ...
        if (this.widget.fieldUiParam.collectionListViewDefinition != null &&
          ...
          if (localFieldValue != null) {
            if (localFieldValue.valueLinkNode != null &&
              ...
              if (refCollectionName == "vwticket") {} else
              if (refCollectionName == "transaksiinduksakti") {
                recordId =
                    this.widget
                        .getFieldvalueCurrentResponseFunction()
                        .recordId;
  ...
  void implementResultCallback(VwRowData result){

    if(result!=null)
      {
        print("VwFormPage result="+ jsonEncode(result.toJson()));
        if(this.widget.fieldValue.valueNodeList==null)
        {
          this.widget.fieldValue.valueNodeList=[];
        }
        this.widget.fieldValue.valueNodeList!.add(new VwNode(nodeType: VwNode.ntnRowData, displayName:  "<invalid_display_name>", ownerUserId: "<invalid_user_id>", recordId: result.recordId,content: new VwNodeContent( rowData: result)));
      }
  ...
  Widget createNewNode(BuildContext context)
  {
    Widget returnValue = Container();
    try
        {
          String? formDefinitionId=this.widget.formField.fieldUiParam.formDefinitionId;

          if(formDefinitionId!=null)
            {
             returnValue=VwFormSubmitPage(
                key: UniqueKey(),
                defaultFormDefinitionIdList:[formDefinitionId],
                appInstanceParam: widget.appInstanceParam,
                resultCallback: this.implementResultCallback,
  ...
  List<VwLinkNode> _createSelectedList(){
    List<VwLinkNode> returnValue=[];
    try
    {
      if(this.widget.fieldValue.valueTypeId==VwFieldValue.vatValueLinkNodeList && this.widget.fieldValue.valueLinkNodeList!=null)
      {

        returnValue = this.widget.fieldValue.valueLinkNodeList!;

      }
      else if(this.widget.fieldValue.valueTypeId==VwFieldValue.vatNodeList)
      {
        returnValue = NodeUtil.createLinkNodeListFormNodeList(nodeList: this.widget.fieldValue.valueNodeList!);
      }
    }
    catch(error)
    {
